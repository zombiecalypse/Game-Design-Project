...
describe Dsl::Event do
  ...
  it "has `on_hit` to automatically run on box collision" do
    ...
    event.hit.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found")]])
  end
  ...
  it "has `on_activate` to run on box collision when clicked" do
    ...
    event.activate.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found")]])
  end
  ...
  it "has `automatically` to run at the beginning of the scene" do
    ...
    event.enter_scene.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found")]])
  end
  ...
  it "allows multiple text popups" do
    ...
    event.hit.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found"), Events::Popup.new(lines: "requires only 3 sacrificed unicorns")]])
  end
  ...
  it "should have the option to run only once" do
    ...
    event.hit.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found")]])
    event.hit.collect(&:lines).should eq([])
  end
  ...
  it "allows to code sequential events" do
    ...
    event.hit.collect(&:lines).should eq([[Events::Popup.new(lines: "Cure for cancer found")]])
    event.hit.collect(&:lines).should eq([[Events::Popup.new(lines: "requires only 3 sacrificed unicorns")]])
  end
end
